...
function cycleEvents(index)
{
    ...
    {
        $('#events-table').html(returnEventsTable(eventdata.aarstore[index]));
        $('#events-table').fadeIn(300, function() {
...
function returnEventsTable(data)
{
    ...
    text = '<table class="m-2 text-white"><tr><td class="pb-2" colspan="'+(2 * ct + 1)+'"><h6 class="mb-0"><b>'+data.ap.icao+'</b> '+data.ap.name+'<br><small class="text-muted">'+city+'</small></h6></td></tr><tr style="font-family: \'JetBrains Mono\'"><td><div style="width: 1px; height: 30px; background-color: #888; margin: 0 auto"></div></td>';
    for(element in data.aar)
    {
        text += '<td style="min-width: 30px; text-align: center"><h5 class="mb-0">'+data.aar[element][2]+'</h5></td><td><div style="width: 1px; height: 30px; background-color: #888; margin: 0 auto"></div></td>';
    }
